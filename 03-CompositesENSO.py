...
def SeasonalPlot(Precipitation, axes, *args):
    ...
    for k, season in enumerate(('DJF', 'MAM', 'JJA', 'SON')):
        ...
        cs = Precipitation.sel(season=season).plot.contourf(
            ax=axes[args[0], k], vmin=-10, vmax=10, levels=levels,
            cmap='bwr_r', add_colorbar=False,
...
def main():
    ...
    #List files
    FileDirectionsCMIP = np.sort(
        np.array(glob.glob(InDirectoryCMIP5 + "*.nc")))
    FileDirectionsONIs = np.sort(
        np.array(glob.glob(InDirectoryONIsCMIP5 + "*.csv")))
    ...
    RegionToPlot = np.array([[-10, 17],
                             [-100, -65]])
    #Define wheter compose for El Nino or La Nina
    ENSO = 'Nina'
    #Model list
    ...
    for i, ModelNameDirection in enumerate(FileDirectionsCMIP):
        ...
        PrecipitationModel = SettingUpXarray(ModelNameDirection,
                                             ModelVarName)
        #Open ONI
        ONIModel = pd.read_csv(FileDirectionsONIs[i], sep=',', index_col=0)
        ...
        #Spatial slicing
        SlicedPrecipitationModel = SlicenetCDFXarray(PrecipitationModel,
                                                     RegionToPlot)
        print('Computing composite for %s event'%ENSO)
        CompENSOMean, CompENSOSTD, N_ENSO = CompositeCMIP(
            SlicedPrecipitationModel, ONIModel, ENSO)
        print('Computing Long term climatologies')
        LongTermClim_Mean_MOD = PrecipitationModel.groupby(
            'time.season').mean('time')
        print('Plotting data')
        SeasonalPlot(CompENSOMean-LongTermClim_Mean_MOD,
                     axes, i, ModelNames[i])
    print('Saving figure')
    plt.tight_layout()
    fig.subplots_adjust(wspace=0, hspace=0)
    fig.savefig(OutDirectoryFigures+'CompositesPanel_Nino.pdf',
                format='pdf',
